...
class PredictionResponse(BaseModel):
    prediccion_ml: float = Field(..., description="Predicción del Agente ML (XGBoost)")
    ...
    ensemble_w_xgb: Optional[float] = Field(None, description="Peso XGBoost en el ensemble (ajustado por Agente 6)")
    ensemble_w_lstm: Optional[float] = Field(None, description="Peso LSTM en el ensemble (ajustado por Agente 6)")
    regimen_epidemico: Optional[str] = Field(None, description="Régimen epidémico detectado por Agente 6")
    regimen_descripcion: Optional[str] = Field(None, description="Descripción del régimen epidémico")
